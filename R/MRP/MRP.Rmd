...
fit <- stan_glmer(
  votes ~ factor(Un_M) +  
    (1 |  Count_psns_occ_priv_dwgs_M) + (1 | Indigenous_P_Tot_M) + (1 | Birthplace_Australia_M), ## include intercepts in the cells based on data we have, but are not using 
...
### Gaussian distribution functions now

posterior_prob <- posterior_linpred(fit, transform = TRUE, newdata = census)
